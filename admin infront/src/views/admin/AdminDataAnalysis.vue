	...
	<div class="admin-data-analysis-container">
		...
		<div class="page-header">
			<div class="header-content">
				<div class="header-left">
					<h1>
						<el-icon><TrendCharts /></el-icon> 数据分析中心
					</h1>
			...
			<el-row :gutter="20">
				<el-col :xs="24" :sm="12" :md="6" :lg="6">
					<div class="metric-card" style="border-left-color: #fe2c55">
						<div
							class="metric-icon"
							style="background: linear-gradient(135deg, #fe2c55, #ff6b8b)"
						>
							<el-icon><VideoPlay /></el-icon>
						</div>
						<div class="metric-info">
							<div class="metric-value">
								{{ formatNumber(coreMetrics.totalViews) }}
							</div>
							<div class="metric-label">总播放量</div>
							<div
								class="metric-trend"
								:class="coreMetrics.viewTrend >= 0 ? 'up' : 'down'"
							>
								<el-icon :size="12"
									><TrendingUp v-if="coreMetrics.viewTrend >= 0" /><TrendingDown
										v-else
								/></el-icon>
				...
				<el-col :xs="24" :sm="12" :md="6" :lg="6">
					<div class="metric-card" style="border-left-color: #25f4ee">
						<div
							class="metric-icon"
							style="background: linear-gradient(135deg, #25f4ee, #6cebff)"
						>
							<el-icon><Star /></el-icon>
						</div>
						<div class="metric-info">
							<div class="metric-value">
								{{ formatNumber(coreMetrics.totalLikes) }}
							</div>
							<div class="metric-label">总点赞数</div>
							<div
								class="metric-trend"
								:class="coreMetrics.likeTrend >= 0 ? 'up' : 'down'"
							>
								<el-icon :size="12"
									><TrendingUp v-if="coreMetrics.likeTrend >= 0" /><TrendingDown
										v-else
								/></el-icon>
				...
				<el-col :xs="24" :sm="12" :md="6" :lg="6">
					<div class="metric-card" style="border-left-color: #00c864">
						<div
							class="metric-icon"
							style="background: linear-gradient(135deg, #00c864, #85d475)"
						>
							<el-icon><Share /></el-icon>
						</div>
						<div class="metric-info">
							<div class="metric-value">
								{{ formatNumber(coreMetrics.totalShares) }}
							</div>
							<div class="metric-label">总收藏数</div>
							<div
								class="metric-trend"
								:class="coreMetrics.shareTrend >= 0 ? 'up' : 'down'"
							>
								<el-icon :size="12"
									><TrendingUp
										v-if="coreMetrics.shareTrend >= 0" /><TrendingDown v-else
								/></el-icon>
				...
				<el-col :xs="24" :sm="12" :md="6" :lg="6">
					<div class="metric-card" style="border-left-color: #ff9500">
						<div
							class="metric-icon"
							style="background: linear-gradient(135deg, #ff9500, #ffb74d)"
						>
							<el-icon><ChatLineRound /></el-icon>
						</div>
						<div class="metric-info">
							<div class="metric-value">
								{{ formatNumber(coreMetrics.totalComments) }}
							</div>
							<div class="metric-label">总评论数</div>
							<div
								class="metric-trend"
								:class="coreMetrics.commentTrend >= 0 ? 'up' : 'down'"
							>
								<el-icon :size="12"
									><TrendingUp
										v-if="coreMetrics.commentTrend >= 0" /><TrendingDown v-else
								/></el-icon>
			...
			<el-row :gutter="20">
				...
				<el-col :xs="24" :lg="12">
					<div class="chart-card">
						<div class="chart-header">
							<h3>
								<el-icon><TrendCharts /></el-icon> 播放量趋势
							</h3>
							<div class="chart-actions">
								<el-select
									v-model="viewChartType"
									size="small"
									style="width: 120px"
									@change="handleViewChartTypeChange"
								>
				...
				<el-col :xs="24" :lg="12">
					<div class="chart-card">
						<div class="chart-header">
							<h3>
								<el-icon><PieChart /></el-icon> 用户互动分析
							</h3>
							<div class="chart-actions">
								<el-radio-group
				...
				<el-col :xs="24" :lg="12">
					<div class="chart-card">
						<div class="chart-header">
							<h3>
								<el-icon><User /></el-icon> 用户增长趋势
							</h3>
							<div class="chart-actions">
								<div class="chart-tabs">
									<el-radio-group
										v-model="userGrowthType"
										size="small"
										@change="handleUserGrowthTypeChange"
									>
				...
				<el-col :xs="24" :lg="12">
					<div class="chart-card">
						<div class="chart-header">
							<h3>
								<el-icon><DataAnalysis /></el-icon> 视频分类分布
							</h3>
							<div class="chart-actions">
								<el-select
									...
									size="small"
									style="width: 120px"
									@change="handleCategorySortChange"
								>
				...
				<el-col :xs="24" :lg="24">
					<div class="chart-card">
						<div class="chart-header">
							<h3>
								<el-icon><Clock /></el-icon> 用户活跃时段分析
							</h3>
							<div class="chart-actions">
								<el-radio-group
				...
				<el-col :xs="24" :lg="24">
					<div class="table-card">
						<div class="table-header">
							<h3>
								<el-icon><Trophy /></el-icon> 热门视频排行榜
							</h3>
							<div class="table-actions">
								<el-select
									v-model="hotVideoSort"
									size="small"
									style="width: 120px"
									@change="handleHotVideoSortChange"
								>
									<el-option label="按播放量" value="views" />
						...
						<div class="table-container">
							...
							>
								...
								<el-table-column label="互动率" width="100" align="center">
									<template #default="scope">
										<span class="interaction-rate"
											>{{ (scope.row.interactionRate * 100).toFixed(1) }}%</span
										>
									</template>
								</el-table-column>
								...
								<el-table-column label="趋势" width="80" align="center">
									<template #default="scope">
										<div
											class="video-trend"
											:class="scope.row.trend >= 0 ? 'up' : 'down'"
										>
											<el-icon :size="12"
												><TrendingUp v-if="scope.row.trend >= 0" /><TrendingDown
													v-else
											/></el-icon>
		...
		>
			...
			<div class="zoom-chart-actions">
				...
				<el-button :icon="Refresh" @click="refreshZoomChart" size="small">
					刷新数据
				</el-button>
			</div>
		</el-dialog>
...
// 计算平均数据
fixedActivePeriodData.all = fixedActivePeriodData.weekday.map(
	(weekdayValue, index) => {
		return Math.round(
			(weekdayValue + fixedActivePeriodData.weekend[index]) / 2,
		);
	},
);
...
const initAllCharts = () => {
	...
	// 用户互动分析图
	interactionChartInstance = initChart(
		interactionChartRef,
		getInteractionChartOption(),
	);

	// 用户增长趋势图
	userGrowthChartInstance = initChart(
		userGrowthChartRef,
	...
	// 用户活跃时段图
	activePeriodChartInstance = initChart(
		activePeriodChartRef,
		getActivePeriodChartOption(),
	);
};
...
const getUserGrowthChartOption = (isZoom = false) => {
	...
	return {
		...
		series: [
			{
				name:
					userGrowthType.value === "new"
						? "新增用户"
						: userGrowthType.value === "active"
							? "活跃用户"
							: "累计用户",
				type: "line",
				smooth: true,
				...
				areaStyle: {
					color: {
						...
						colorStops: [
							{
								offset: 0,
								color: colors[userGrowthType.value] + "40",
							},
							{
								offset: 1,
								color: colors[userGrowthType.value] + "05",
							},
						],
					},
				},
...
// 播放量趋势类型改变
const handleViewChartTypeChange = async () => {
	await loadViewTrend();
};

// 互动分析类型改变
const handleInteractionTypeChange = async () => {
	await loadInteraction();
};

// 用户增长类型改变
const handleUserGrowthTypeChange = async () => {
	await loadUserGrowth();
};

// 分类排序改变
const handleCategorySortChange = async () => {
	await loadCategoryDistribution();
};

// 活跃时段改变
const handleActivePeriodChange = async () => {
...
const exportData = async () => {
	try {
		...
		const data = await dataAnalysisApi.exportData(params);
...
const buildTimeRangeParams = () => {
	...
	if (timeRange.value === "custom" && customDateRange.value && customDateRange.value.length === 2) {
		params.startDate = dataAnalysisApi.formatDateForBackend(customDateRange.value[0]);
		params.endDate = dataAnalysisApi.formatDateForBackend(customDateRange.value[1]);
	}
...
const loadCoreMetrics = async () => {
	try {
		...
		console.log("Loading core metrics with params:", params);
		const data = await dataAnalysisApi.getCoreMetrics(params);
		console.log("Core metrics data received:", data);
		...
		if (data && typeof data === 'object') {
			coreMetrics.value = {
				...
				likeTrend: data.likeTrend || 0,
				totalComments: data.totalComments || 0,
				commentTrend: data.commentTrend || 0,
				totalShares: data.totalShares || 0,
				shareTrend: data.shareTrend || 0,
			};
			console.log("Core metrics updated:", coreMetrics.value);
		} else {
		...
		ElMessage.error("加载核心指标失败: " + (error.message || "未知错误"));
	}
};
...
const loadViewTrend = async () => {
	try {
		const params = {
			...buildTimeRangeParams(),
			chartType: viewChartType.value,
		};
		const data = await dataAnalysisApi.getViewTrend(params);
		
		// 更新图表
		if (viewChartInstance && data.labels && data.data) {
		...
		console.error("Failed to load view trend:", error);
		ElMessage.error("加载播放量趋势失败");
	}
};
...
const loadInteraction = async () => {
	try {
		...
		const data = await dataAnalysisApi.getInteraction(params);
		...
		if (interactionChartInstance) {
			if (interactionType.value === "distribution") {
				...
				interactionChartInstance.setOption({
					series: [{
						data: [
							{ value: data.comments, name: "评论" },
							{ value: data.likes, name: "点赞" },
							{ value: data.collects, name: "收藏" },
						],
					}],
				});
			} else {
...
const loadUserGrowth = async () => {
	try {
		const params = {
			...buildTimeRangeParams(),
			type: userGrowthType.value,
		};
		const data = await dataAnalysisApi.getUserGrowth(params);
		
		// 更新图表
		if (userGrowthChartInstance && data.labels && data.data) {
...
const loadCategoryDistribution = async () => {
	try {
		...
		const data = await dataAnalysisApi.getCategoryDistribution(params);
		
		// 更新图表
		if (categoryChartInstance && data.categories) {
...
const loadActivePeriod = async () => {
	try {
		...
		const data = await dataAnalysisApi.getActivePeriod(params);
		
		// 更新图表
		if (activePeriodChartInstance && data.hours && data.data) {
			activePeriodChartInstance.setOption({
...
const loadHotVideos = async () => {
	try {
		const params = {
			...buildTimeRangeParams(),
			sortBy: hotVideoSort.value,
			limit: 10,
		};
		const data = await dataAnalysisApi.getHotVideos(params);
		
		// 转换数据格式
		const videos = dataAnalysisApi.transformHotVideos(data);
		rawHotVideos.value = videos;
	} catch (error) {
		console.error("Failed to load hot videos:", error);
		ElMessage.error("加载热门视频排行失败");
	}
};
